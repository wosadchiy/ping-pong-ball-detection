...
def create_ui(store):
    ...
    with dpg.window(label="Dashboard", width=300, height=800, pos=[0, 0], no_close=True, no_move=True):
        ...
        with dpg.collapsing_header(label="HSV SETTINGS", default_open=True):
        ...
        with dpg.collapsing_header(label="MOTOR", default_open=True):
            dpg.add_checkbox(label="TRACKING", default_value=store.is_tracking, callback=lambda s, v: setattr(store, 'is_tracking', v))
            dpg.add_slider_float(label="Kp", min_value=0.0, max_value=5.0, default_value=store.kp, callback=lambda s, v: setattr(store, 'kp', v))
            dpg.add_slider_int(label="MaxV", min_value=30, max_value=100, default_value=int(store.max_omega), callback=lambda s, v: setattr(store, 'max_omega', float(v)))
            dpg.add_button(label="SAVE SETTINGS", width=-1, callback=store.save_to_json)

    with dpg.window(label="Camera", pos=[310, 0], no_close=True):
        dpg.add_image("camera_texture")
    with dpg.window(label="Mask", tag="mask_window", pos=[310, 520], show=False):
        dpg.add_image("mask_texture")

    # КЛЮЧЕВОЕ: vsync=False убирает зависимость от монитора
    dpg.create_viewport(title='BallTracker HighSpeed', width=1000, height=900, vsync=False)
    dpg.setup_dearpygui()
    dpg.show_viewport()

# Выносим константу, чтобы не считать её в цикле
INV_255 = np.float32(1.0 / 255.0)

def update_texture(tag, frame):
    # Максимально быстрая конвертация через NumPy
